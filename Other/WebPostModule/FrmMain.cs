...
namespace WebPostModule
{
    public partial class FrmM : Form
    {
        ...
        private void ShowModuleInfo(ModuleInfo mi)
        {
            ...
            if (!string.IsNullOrEmpty( mi.Password))
            {
            ...
            if (IsAuth)
            {
                ...
                #region 登陆部分
                rtbeLoginUrl.TextValue = mi.LoginUrl;
                rtbeLoginRefer.TextValue = mi.LoginRefer;
                rtbeLoginImgUrl.TextValue = mi.LoginImgUrl;
                ...
                txtLoginErrInfo.Text = string.Join("\r\n", mi.LoginErrInfo);
                txtLoginImgErrInfo.Text = string.Join("\r\n", mi.LoginImgErrInfo);
                #endregion


                #region 获取栏目列表
                rtbeRefreshUrl.TextValue = mi.RefreshUrl;
                rtbeRefreshRefer.TextValue = mi.RefreshRefer;
                rtbeRefreshStart.TextValue = mi.RefreshStart;
                rtbeRefreshEnd.TextValue = mi.RefreshEnd;
                rtbeRefreshRegex.TextValue = mi.RefreshRegex;
                #endregion

                #region 网页随机值获取
                foreach (var item in mi.HashDic)
                {
                ...
                #region 内容发布参数
                rtbePostUrl.TextValue = mi.PostUrl;
                rtbePostRefer.TextValue = mi.PostRefer;
                foreach (var item in mi.PostData)
                {
                    ListViewItem li = new ListViewItem(new string[] { item.Key, item.Value });
                    lvPostData.Items.Add(li);
                }
                txtPostSuccessInfo.Text = string.Join("\r\n", mi.PostSuccessInfo); 
                txtPostErrInfo.Text = string.Join("\r\n", mi.PostErrInfo);
                #endregion
            }
            else
            {
            ...
            #region 模块说明
            txtMemo.Text = mi.Memo;
            txtCopyRightInfo.Text = mi.CopyRightInfo;
            txtCopyRightLink.Text = mi.CopyRightLink;
            if (IsAuth)
            {
                txtPassword.Text = mi.Password;
            }
            #endregion
        ...
        private ModuleInfo GetModuleInfo()
        {
            ...
            #region 登陆部分
            mi.LoginUrl= rtbeLoginUrl.TextValue;
            mi.LoginRefer= rtbeLoginRefer.TextValue;
            mi.LoginImgUrl= rtbeLoginImgUrl.TextValue;

            List<PostData> listLoginpost = new List<PostData>();
            foreach (ListViewItem item in lvLoginPost.Items)
            {
                listLoginpost.Add(new PostData() {  Key=item.SubItems[0].Text,Value=item.SubItems[1].Text});
            }
            mi.LoginPost = listLoginpost.ToArray();

            mi.LoginSuccessInfo=txtLoginSuccessInfo.Text.Split(new string[] {"\r\n" },StringSplitOptions.RemoveEmptyEntries);
            mi.LoginErrInfo = txtLoginErrInfo.Text.Split(new string[] { "\r\n"},StringSplitOptions.RemoveEmptyEntries);
            mi.LoginImgErrInfo = txtLoginImgErrInfo.Text.Split(new string[] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries); ;
            #endregion


            #region 获取栏目列表
            mi.RefreshUrl= rtbeRefreshUrl.TextValue;
            mi.RefreshRefer=rtbeRefreshRefer.TextValue;
            mi.RefreshStart=rtbeRefreshStart.TextValue;
            mi.RefreshEnd= rtbeRefreshEnd.TextValue;
            mi.RefreshRegex= rtbeRefreshRegex.TextValue;
            #endregion
            ...
            #region 内容发布参数
            mi.PostUrl= rtbePostUrl.TextValue;
            mi.PostRefer= rtbePostRefer.TextValue;
            List<PostData> listpost = new List<PostData>();
            ...
            mi.PostData = listpost.ToArray();
            mi.PostSuccessInfo = txtPostSuccessInfo.Text.Split(new string[] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries); ;
            mi.PostErrInfo = txtPostErrInfo.Text.Split(new string[] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries); ;
            #endregion

            #region 模块说明
            mi.Memo= txtMemo.Text;
            mi.CopyRightInfo= txtCopyRightInfo.Text;
            mi.CopyRightLink= txtCopyRightLink.Text;
            mi.Password= txtPassword.Text;
            #endregion
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (btnSave.Tag != null)
            {
                ...
                if (saveFileDialog.FileName.EndsWith(".wpm"))
                {
                    saveFileDialog.FileName = saveFileDialog.FileName.Substring(0, saveFileDialog.FileName.Length - 4) + ".gpm";
                }
            }
