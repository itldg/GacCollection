...
namespace Gac
{
    public class FileDownloader
    {
        ...
        private bool ResumeBrokenTransfer = true;
        /// <summary>
        ///   是否支持断点续传
        /// </summary>
        /// <returns>是否支持断点续传</returns>
        public bool getResumeBrokenTransfer()
        {
            return ResumeBrokenTransfer;
        }
        /// <summary>
        /// 累计已下载大小
        /// </summary>
            ...
            try
            {
                ...
                using (HttpWebResponse response = (HttpWebResponse)request.GetResponse())
                {
                    if (response.StatusCode == HttpStatusCode.OK)
                    {
                        ...
                        if (string.IsNullOrEmpty(filename))
                        {
                            filename = Uri.UnescapeDataString(Path.GetFileName(response.ResponseUri.ToString()));//获取文件名称 uri 解码中文字符
                        }
                        this.fileSize = response.ContentLength;//根据响应获取文件大小
                        if (this.fileSize <= 0) throw new Exception("获取文件大小失败");
                        if (filename.Length == 0|| !IsValidFileName(filename)) throw new Exception("获取文件名失败");
                        if (response.Headers["Content-Range"] == null&&response.Headers["Accept-Ranges"] == null)
                        {
                            this.ResumeBrokenTransfer = false;
                        }
                        fileName = filename;
